...
class DataPersistenceService:
    ...
    def _init_database(self):
        ...
        try:
            ...
            # Create video_analytics table for aggregated data
            cursor.execute('''
            ...
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_video_results_job_id ON video_results(job_id)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_video_results_file_hash ON video_results(file_hash)')
            cursor.execute('CREATE INDEX IF NOT EXISTS idx_analytics_date ON video_analytics(date)')
    ...
    def save_video_result(self, job_id: str, file_hash: str, filename: str,
                         video_metadata: Dict, emotion_analysis: Dict, 
                         video_score: Dict, processing_mode: str) -> bool:
        """
        ...
        try:
            ...
            cursor.execute('''
                ...
                (job_id, file_hash, filename, video_metadata, emotion_analysis, 
                 video_score, processing_mode)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (
                job_id,
                file_hash,
            ...
            logger.info(f"Saved video result for job: {job_id}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to save video result: {str(e)}")
            return False
    ...
    def get_video_result(self, job_id: str) -> Optional[Dict]:
        """
        Get video processing result by job ID
        """
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            ...
            cursor.execute('''
                SELECT file_hash, filename, video_metadata, emotion_analysis,
                       video_score, processing_mode, created_at
                FROM video_results
                WHERE job_id = ?
            ''', (job_id,))
            ...
            if result:
                return {
                    "file_hash": result[0],
                    "filename": result[1],
                    "video_metadata": json.loads(result[2]),
                    "emotion_analysis": json.loads(result[3]),
                    "video_score": json.loads(result[4]),
            ...
            logger.error(f"Failed to get video result: {str(e)}")
            return None
    ...
    def get_analytics(self, days: int = 30) -> Dict:
        ...
        try:
            ...
            # Get daily analytics
            cursor.execute('''
                SELECT date, total_videos, successful_processing, failed_processing,
                       total_processing_time, average_processing_time, total_file_size,
                       unique_videos, cache_hits
        ...
        try:
            ...
            cursor.execute('''
                SELECT 
                    ...
                    AVG(CASE WHEN completed_at IS NOT NULL AND started_at IS NOT NULL 
                        ...
                        ELSE NULL END) as avg_processing_time,
                    SUM(file_size) as total_file_size,
                    COUNT(DISTINCT file_hash) as unique_videos
                FROM processing_jobs
            ...
            if stats and stats[0] > 0:  # Only update if there are jobs for this date
                cursor.execute('''
                    ...
                    (date, total_videos, successful_processing, failed_processing,
                     total_processing_time, average_processing_time, total_file_size, unique_videos)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (date, *stats))
    ...
    def cleanup_old_data(self, days_to_keep: int = 90) -> int:
        ...
        try:
            ...
            # Delete old video results (orphaned)
            cursor.execute('''
